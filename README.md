...
* [Word](https://github.com/mit-plv/coqutil/tree/master/src/coqutil/Word): Lemmas and automation (ring, endianness conversions, simplification) for fixed-width machine words represented as the standard library's `bits width` (`Zmod (2^width)`), for a potentially abstract bit width.
