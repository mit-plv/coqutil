(* Instead of using getters created on the spot of the form
   (match r with {| fieldName := x |} => x end),
   this file uses coqutil.Tactics.RecordEta, which reuses
   the getters that Coq creates when defining records.
   This results in smaller terms for setters, but in the
   Softmul proof, it made `change` and `Qed` much slower,
   probably because the conversion algorithm hesitates
   to unfold named getters and thus fails to see the simplification
   opportunity from the match in the getter's definition. *)
