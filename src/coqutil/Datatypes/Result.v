...
Notation "'error:(' x )" := (Failure (dcons x%string dnil))
  (at level 0, x at level 0, format "'error:(' x )")
  : error_scope.

Notation "'error:(' x y .. z )" := (Failure (dcons x (dcons y .. (dcons z dnil) ..))%string)
  (at level 0, x at level 0, y at level 0, z at level 0, format "'error:(' x  y  ..  z )")
  : error_scope.

Goal @id (result nat) error:(4 "is not a" bool) =
     Failure (dcons 4 (dcons "is not a"%string (dcons bool dnil))).
Proof. reflexivity. Abort.

Goal error:("just one string") = @Failure nat (dcons "just one string"%string dnil).
Proof. reflexivity. Abort.
