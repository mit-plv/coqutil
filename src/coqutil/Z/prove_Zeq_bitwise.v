...
Proof.
  ...
  - rewrite Z.testbit_true in E by lia.
    assert (a < -1 \/ a = -1 \/ 0 <= a) as C by lia. destruct C as [C | [C | C]].
    + apply Z.bits_above_log2_neg; [lia|].
      rewrite (Z.pow_lt_mono_r_iff 2) by lia.
      pose proof (Z.log2_spec (Z.pred (- a))) as P.
      pose proof (Z.pow_lt_mono_r 2 l i).
      lia.
    + subst. apply Z.bits_m1. lia.
    + rewrite Z.div_small in E by lia.
      cbv in E. discriminate.
  - assert (a < 0 \/ 0 <= a) as C by lia. destruct C as [C | C].
    + exfalso.
      pose proof (Z.testbit_true a l) as P.
      destruct P as [_ P]; [lia|].
      rewrite P in E; [discriminate|clear P E].
      rewrite Zdiv_small_neg by lia.
      reflexivity.
    + assert (a / 2 ^ i = 0) as F. {
        pose proof (Z.pow_lt_mono_r 2 l i).
        apply Z.div_small. lia.
      }
      rewrite Z.testbit_false by lia.
      rewrite F.
      reflexivity.
Qed.
...
Proof.
  ...
  apply (@testbit_above (Z.log2_up b) n); lia.
Qed.
...
Proof.
  intros.
  pose proof (Z.log2_log2_up_spec b).
  apply testbit_above_signed; lia.
Qed.
...
Proof.
  ...
  - specialize (P H). destruct P as [c P]. subst a. clear H.
    ...
    + replace p with ((1 + (p - i - 1)) + i) by lia.
...
(*   testbit_above_signed''  <-- TODO why does this one make proofs run forever? *)
     mod0_testbit'
     mod20_testbit'
     using (try eassumption; try reflexivity; rewrite? Z.log2_up_pow2 by lia; lia)
...
Ltac prove_Zeq_bitwise_pre :=
  rewrite ?signExtend_alt_bitwise by lia; unfold signExtend_bitwise in *;
  subst;
