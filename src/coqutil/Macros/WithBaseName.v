...
Notation "& , x" := (ltac:(constr_string_basename_of_constr_reference_cps x ltac:(fun s => exact s)), x)
  (at level 9, x global, only parsing).

Local Ltac2 rec named_list (xs : constr) :=
 match! xs with
 | @cons ?t ?x ?xs =>
     let nx := constr_string_basename_of_constr_reference x in
...
Goal True.
  ...
  pose (&[, unit ]).
  pose (&[, unit ; nat ]).
  unify (&[, unit ; nat ]) [("unit", unit); ("nat", nat)].
Abort.
